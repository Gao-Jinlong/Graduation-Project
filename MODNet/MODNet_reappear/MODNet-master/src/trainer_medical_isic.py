...
if __name__ == '__main__':
    from matting_dataset_medical_isic import MattingDataset, Rescale, \
        ToTensor, Normalize, ToTrainArray, \
        ConvertImageDtype, GenTrimap,GetRandomData
...
#-----------------------------------------------------------------------
    ...
    # 数据加载对象
    num_workers = 1  # 多线程加载数据
    train_dataset = MattingDataset(data_type='Training', transform=transform_train)
    train_data = DataLoader(train_dataset,
                            batch_size=BS,  # how many samples per batch to load
                            shuffle=True,
                            drop_last=True
                            )  # 乱序
    val_dataset = MattingDataset(data_type='Test', transform=transform_val)
