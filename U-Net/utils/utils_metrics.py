...
def Iou_score(smooth = 1e-5, threhold = 0.5):
    def _Iou_score(y_true, y_pred):
        ...
        intersection = backend.sum(y_true[..., :-1] * y_pred, axis = [0, 1, 2])  #  舍弃最后一个元素
...
def per_class_iu(hist):
    return np.diag(hist) / np.maximum((hist.sum(1) + hist.sum(0) - np.dig(hist)), 1)
...
def compute_mIoU(gt_dir, pre_dir, png_name_list, num_classes, name_classes):
    ...
    for ind in range(len(gt_imgs)):
        #  读取一张图像分割结果，转换成numpy数组
        pred = np.array(Image.open(pred_imgs[ind]))
        #  读取一张对应的标签，转换为numpy数组
        label = np.array(Image.open(gt_imgs[ind]))

        #如果图像分割结果与标签大小不一样，这张图片就不计算
    ...
    #  在所有验证集图像上求所有类别平均值的mIoU值，计算时忽略NaN值
...
def show_results(miou_out_path, hist, IoUs, PA_Recall, Precision, name_classes, tick_font_size = 12):
    ...
    raw_plot_func(PA_Recall, name_classes, "mPA = {0:.2f}%".format(np.nanmean(PA_Recall) * 100), "Pixel Accuracy", \
