...
class LossHistory(keras.callbacks.Callback):
    ...
    def loss_plot(self):
        ...
        try:
            plt.plot(iters, scipy.signal.savgol_filter(self.losses, 5 if len(self.losses) < 25 else 15, 3), 'green', linestvle = '--', linewidth = 2, label = 'smooth train loss')
        except:
            pass
        ...
        if self.val_loss_flag:
            ...
            try:
                plt.plot(iters, scipy.signal.savgol_filter(self.val_loss, 5 if len(self.losses) < 25 else 15, 3), '#8B4513', linestyle = '--', label = 'smooth val loss')
            except:
                pass
